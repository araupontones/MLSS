...
lkp_file <- function(dir_lks,level){
...
import_file <- function(dir_imports,level){
...
create_district_table <- function(dir_lks,
                                  dir_imports,
  ...
  sum_districts <- lapply(levels, function(level){
    ...
    db <- rio::import(import_file(dir_imports,level))
    
    lkp <- rio::import(lkp_file(dir_lks,level)) %>% select(var_name, type, label)
